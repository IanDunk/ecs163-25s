...
const originalState = {
    selectedXAttribute: 'Attack',
    selectedYAttribute: 'Defense',
    selectedGenerations: [1, 2, 3, 4, 5, 6],
...
let scatterContent; // New group for scatter plot content
...
function resizeVisualizations() {
    ...
    // Update zoom rect and clip path dimensions
    g1.select(".zoom-rect")
        .attr("width", scatterContentWidth)
        .attr("height", scatterPlotContentHeight);
    
    g1.select(".scatter-border")
        .attr("width", scatterContentWidth)
        .attr("height", scatterPlotContentHeight);
    ...
    if (zoom) {
        zoom.extent([[0, 0], [scatterContentWidth, scatterPlotContentHeight]])
            .translateExtent([[-scatterContentWidth, -scatterPlotContentHeight], 
                             [scatterContentWidth * 2, scatterPlotContentHeight * 2]]);
...
// Function to reset all graph settings to original state
function resetGraph() {
    // Reset state variables
    selectedXAttribute = originalState.selectedXAttribute;
    selectedYAttribute = originalState.selectedYAttribute;
    selectedGenerations = [...originalState.selectedGenerations];
    showLegendaryOnly = originalState.showLegendaryOnly;
    selectedTypes = new Set(originalState.selectedTypes);
    
    // Reset UI elements
    xAxisSelect.property("value", selectedXAttribute);
    yAxisSelect.property("value", selectedYAttribute);
    
    // Reset generation checkboxes
    d3.selectAll(".generation-checkbox").property("checked", function() {
        return selectedGenerations.includes(+this.value);
    });
    
    // Reset type legend selections
    d3.selectAll(".type-legend-item .type-legend-box").classed("selected", false);
    d3.select(".legendary-item .type-legend-box").classed("selected", false);
...
d3.csv("pokemon_alopez247.csv").then(rawData => {
    ...
    // Add clipping path for scatter plot
    ...
    function zoomed() {
        ...
        const newX = currentZoom.rescaleX(x1);
        const newY = currentZoom.rescaleY(y1);
        ...
        scatterContent.selectAll("circle")
            .attr("cx", d => newX(d[selectedXAttribute]))
            .attr("cy", d => newY(d[selectedYAttribute]));
    }

    // Create zoom behavior
    zoom = d3.zoom()
        .scaleExtent([0.5, 20]) // Min and max zoom levels
        .extent([[0, 0], [scatterContentWidth, scatterPlotContentHeight]])
        .translateExtent([[-scatterContentWidth, -scatterPlotContentHeight], 
                         [scatterContentWidth * 2, scatterPlotContentHeight * 2]])
        .on("zoom", zoomed);

    // Add zoom rect to capture events
    ...
    // Apply zoom behavior to the entire scatter plot group
    g1.call(zoom);

    // Add border to scatter plot area
    g1.append("rect")
        .attr("class", "scatter-border")
        .attr("width", scatterContentWidth)
        .attr("height", scatterPlotContentHeight)
        .style("fill", "none")
        .style("stroke", "#dee2e6")
    ...
    // Create content group for scatter plot that will be clipped
    scatterContent = g1.append("g")
    ...
    xAxisSelect.on("change", function() {
        ...
        // Reset zoom when changing axis attributes since the data scale changes
        resetZoom();
        filterAndDrawAll();
    });
    ...
    // Reset graph button listener
    d3.select("#reset-graph-btn").on("click", function() {
        resetGraph();
    });
...
function updateScatterScalesAndAxes() {
    ...
    if (currentZoom.k !== 1 || currentZoom.x !== 0 || currentZoom.y !== 0) {
        // If zoomed, update axes to reflect current zoom level without animation
        const transformedX = currentZoom.rescaleX(x1);
        const transformedY = currentZoom.rescaleY(y1);
        xAxisG.call(d3.axisBottom(transformedX));
        yAxisG.call(d3.axisLeft(transformedY));
    } else {
        // If not zoomed, animate to base scale
...
function updateScatterPlot(data) {
    ...
    const circles = scatterContent.selectAll("circle").data(displayData, d => d.Number);

    circles.exit().transition().duration(500).attr("r", 0).remove();
    
    circles.enter().append("circle")
        .attr("r", 0)
        .attr("cx", d => transformedX(d[selectedXAttribute]))
        .attr("cy", d => transformedY(d[selectedYAttribute]))
        .merge(circles)
